...
import com.freeman.sys.component.O2O;
...
public class CacheServiceImpl implements ICacheService {
    ...
    public SysUser getUserInfo(Long id) {
        ...
        List<SysUser> users = cacheable( (List<Long> ids) -> {
            List<SysUserDto> allByTemplate;
            if (0 == ids.size()) {
                allByTemplate = userRepository.findAllByTemplate(); //使用sql模板查询
            } else {
                allByTemplate = userRepository.findAllByTemplate(ids); //使用sql模板查询
            }
            return O2O.INSTANCE.userDto2user(allByTemplate);
    ...
    private List<SysRole> getRoles(List<Long> roleIds) {
        ...
        List<SysRole> roles = cacheable((List<Long> ids) -> {
            List<SysRoleDto> allByTemplate;
            if (0 == ids.size()) {
                allByTemplate = roleRepository.findAllByTemplate();
            } else {
                allByTemplate = roleRepository.findAllByTemplate(ids);
            }
            return O2O.INSTANCE.roleDto2role(allByTemplate);
    ...
    private List<SysPermission> getPermissions(List<Long> permissionIds) {
        ...
        List<SysPermission> permissions = cacheable((List<Long> ids) -> {
            List<SysPermission> allByTemplate;
            if (0 == ids.size()) {
                allByTemplate = permissionRepository.findAllByTemplate();
            } else {
                allByTemplate = permissionRepository.findAllByTemplate(ids);
            }
            return allByTemplate;
    ...
    public void loadCaches() {
        cachePut((List<Long> ids) -> {
            ...
            nqw.select( " su.*,suc.user_id  AS `config.userId`," +
            ...
            List<SysUser> sysUsers = userRepository.findByNativeSql(nqw, SysUser.class);
            ///////////
            //List<SysUserDto> allByTemplate = userRepository.findAllByTemplate();
            //List<SysUser> sysUsers = O2O.INSTANCE.userDto2user(allByTemplate);
            return sysUsers;
        }, Constants.CACHE.USER_PREFIX, null);

        cachePut((List<Long> ids) -> {
            List<SysRoleDto> allByTemplate = roleRepository.findAllByTemplate();
            return O2O.INSTANCE.roleDto2role(allByTemplate);
        }, Constants.CACHE.ROLE_PREFIX, null);

        cachePut((List<Long> ids) -> {
            List<SysPermission> allByTemplate = permissionRepository.findAllByTemplate();
            return allByTemplate;
