...
export default {
  ...
  computed: {
    columns () {
      ...
      return [{ title: '角色', dataIndex: 'name' },
        { title: '描述', dataIndex: 'remark', scopedSlots: { customRender: 'remark' }, width: 350 },
        { title: '创建时间', dataIndex: 'createTime', sorter: true, sortOrder: sortedInfo.columnKey === 'createTime' && sortedInfo.order },
        { title: '修改时间', dataIndex: 'modifyTime', sorter: true, sortOrder: sortedInfo.columnKey === 'modifyTime' && sortedInfo.order },
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          for (let key of that.selectedRowKeys) {
            roleIds.push(that.dataSource[key].id)
          }
          $deleteRoleById({}, {ids: roleIds.join(',')}).then(() => {
