<template>
  <a-drawer title="角色信息" :maskClosable="false" width=650 placement="right" :closable="true"
  @close="close" :visible="roleInfoVisiable"
    style="height: calc(100% - 55px);overflow: auto;padding-bottom: 53px;">
      <p><a-icon type="crown" />&nbsp;&nbsp;角色名称：{{roleInfoData.name}}</p>
      ...
      <p><a-icon type="trophy" />&nbsp;&nbsp;所拥有的权限：
        <a-tree :treeData="menuTreeData" :key="key"
...
<script>
import {$getPermissions, $getRolePerms} from '@/api'
      ...
      if (this.roleInfoVisiable) {
        $getPermissions().then((r) => {
          ...
          $getRolePerms({}, {id: this.roleInfoData.id}).then((r) => {
            ...
            this.checkedKeys.splice(0, checkData.length, ...checkData)
            this.key = +new Date()
          })
        })
      }
