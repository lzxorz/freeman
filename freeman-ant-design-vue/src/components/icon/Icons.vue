<template>
<div>
  <a-input :placeholder="placeholder" :value="localValue">
      <a-icon v-if="localValue" slot="suffix" type="close-circle" @click="$emit('change', '')"/>
      <a-icon slot="addonAfter" type="setting" style="cursor: pointer" @click="show = true"/>
  </a-input>
  <a-modal v-model="show" :width="900" :keyboard="false" :closable="false" :centered="true"
    @ok="ok" @cancel="colse" :maskClosable="false" :mask="false" okText="确认" cancelText="取消">
    <a-tabs>
...
export default {
  name: 'Icons',
  props: {
    placeholder: { type: String, default: '点击右侧按钮选择图标' },
  ...
  computed: { localValue () { return this.value } },
  methods: {
    chooseIcon (icon) {
    ...
    ok () {
      ...
      this.$emit('change', this.choosedIcon)
      this.close()
    }
  },
