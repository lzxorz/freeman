  ...
  <a-layout-sider class="sider" width="273">
    <setting-item title="导航栏颜色">
      <img-checkbox-group @change="setTheme">
        <img-checkbox img="img/side-bar-dark.svg" :checked="dark"
                      value="dark"/>
        <img-checkbox img="img/side-bar-light.svg" :checked="!dark"
                      value="light"/>
      </img-checkbox-group>
    </setting-item>
    ...
    <setting-item title="导航栏位置">
      <img-checkbox-group @change="setLayout">
        <img-checkbox img="img/side-bar-left.svg" :checked="side" value="side"/>
        <img-checkbox img="img/side-bar-top.svg" :checked="!side" value="head"/>
      </img-checkbox-group>
    </setting-item>
    <setting-item>
      <a-list :split="false">
...
export default {
  ...
  methods: {
    ...
    fixSiderbar (checked) {
      this.$store.commit('setting/setFixSiderbar', checked)
    },
    fixHeader (checked) {
      this.$store.commit('setting/setFixHeader', checked)
    },
    updateUserConfig () {
      this.$put('/sys/user/config', {
